...
import DeleteUser from '../composants/b2b/DeleteUser';
  ...
  useEffect( ()=>{
      ...
      const getB2B = async ()=>{
        ...
        let csrfToken = localStorage.getItem('csrfToken');
              await axios.get(process.env.REACT_APP_API_ListB2Bvalid, {withCredentials: true  , headers: {'Authorization':  csrfToken}  })
        .then((response) => {
          setlistB2B( response.data.filter(
              (item) => { return(item.email.toLowerCase().includes(search.toLowerCase())||item.nom_entreprise.toLowerCase().includes(search.toLowerCase())||item.SIRET.toLowerCase().includes(search.toLowerCase())) }    
               )
          )
